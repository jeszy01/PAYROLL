...
```bash
php artisan tinker
>>> \App\Models\User::create(['name' => 'System Admin', 'email' => 'admin@example.com', 'password' => bcrypt('changeme'), 'role' => 'admin']);
```
...
There are exactly two roles, stored as `role` on `users`:
...
- `hr` — day-to-day HR operations (employees, payroll prep/compute,
...
Enforced server-side via the `role:admin` route middleware
